...
protected:
    ...
    uint GetPointSize() const { return m_pointSize; }

    bool IsStepMode() const { return m_stepMode; }
    uint GetCurrentStep() const { return m_currentStep; }

    uint NumberOfFilledPoints() const { return m_points.size() - 1; }
    ...
    void DrawPoints(uint pointSize = 0) const;

public:

    RasterGridRunnable(SDL_Window* w, SDL_Renderer* r, uint pointSize, uint maxPoints = UINT_MAX);
};
