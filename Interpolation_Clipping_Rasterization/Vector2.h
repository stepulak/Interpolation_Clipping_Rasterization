...
struct Vector2 {
    ...
    Vector2<T> CountNormal() const { return Vector2(-y, x); }
    T Dot(const Vector2<T>& v2) const { return x * v2.x + y * v2.y; }
    T GetLength() const { return static_cast<T>(SDL_sqrt(x*x + y*y)); }
    ...
    Vector2<T>& operator=(const Vector2<T>& v2) = default;
    Vector2<T>& operator=(const SDL_Point& p) { x = p.x, y = p.y; return *this; }

    Vector2<T> operator+(const Vector2<T>& v2) const { return Vector2(x + v2.x, y + v2.y); }
    Vector2<T>& operator+=(const Vector2<T>& v2) { *this = v2 + *this; return *this; }

    Vector2<T> operator-(const Vector2<T>& v2) const { return Vector2(x - v2.x, y - v2.y); }
    Vector2<T>& operator-=(const Vector2<T>& v2) { *this = *this - v2; return *this; }
    ...
    bool operator==(const Vector2<T>& v2) const { return x == v2.x && y == v2.y; }
    bool operator!=(const Vector2<T>& v2) const { return !(*this == v2); }
};
